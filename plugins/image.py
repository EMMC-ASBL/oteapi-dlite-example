...
class image(dlite.DLiteStorageBase):
    ...
    def open(self, location, options=None):
        ...
        self.options = Options(options, defaults="as_gray=false;equalize=false")

    def load(self, id=None):  # noqa: ARG002
        """Returns TEMImage instance."""
        as_gray = dlite.asbool(self.options.as_gray)
        data = imread(
            self.location,
            as_gray=as_gray,
            plugin=self.options.get("plugin"),
        )
    ...
    def save(self, inst):
        ...
        imsave(self.location, scaled, plugin=self.options.get("plugin"))
